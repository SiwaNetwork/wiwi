...
## На оборудование ретрансляции фазовой синхронизации по радиолинии

### 1. Цель задания

Целью настоящего технического задания является определение требований к оборудованию, предназначенному для ретрансляции сигнала фазовой синхронизации по профилю ITU-T G.8275.1 (PTP) через радиолинию и транспортную сеть связи, с возможностью интеграции в гетерогенную инфраструктуру, включая оборудование различных производителей.
...
- Формировать и передавать PTP-пакеты по радиолинии и/или проводным интерфейсам;
...
- Принимать PTP-пакеты от Master-модуля;
...
Оборудование должно обеспечивать передачу данных (PTP-пакетов) на внешний радиопередатчик по согласованному интерфейсу для их дальнейшей передачи по радиоканалу.
...
- Обеспечение требуемой дальности радиосвязи (указать в ТЗ отдельно, если известна) при сохранении качества синхросигнала даже при нестабильности модуляции;
...
2. Оборудование обеспечивает заданную дальность радиосвязи (указать отдельно, если есть цифра) при сохранении качества синхросигнала в условиях нестабильной модуляции;
...
## Ключевые отличия от Wi-Fi синхронизации

| Параметр | Wi-Fi синхронизация | Радиосинхронизация |
|----------|-------------------|-------------------|
| **Транспорт** | Wi-Fi 6E (2.4/5/6 ГГц) | Внешний радиопередатчик (2017 МГц) |
| **Интерфейс** | Встроенные Wi-Fi модули | Стандартный интерфейс (Ethernet/CPRI/eCPRI) |
| **Антенны** | Встроенные MIMO 2x2 | Внешние антенны заказчика |
| **Переключение диапазонов** | Быстрое переключение Wi-Fi | Статическая частота 2017 МГц |
| **Сложность** | Высокая (Wi-Fi 6E) | Средняя (стандартные интерфейсы) |
